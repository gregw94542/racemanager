...
function input_entry( $race_id,$division_id,$heatnum,$skate_id,$pos,
	  $min,$sec,$hun,$place, $points,$notes)
{
		...
		$query = "insert into raw_results
		  (division_id, skater_id, heatnum, 
		   min, sec, hun, pos, point, notes, place)
		  values
                  ($division_id, $skate_id, $heatnum, '$min', '$sec', 
	           '$hun', '$pos', '$points', '$notes', '$place')";
		#echo "$query<br>";
		mysql_query($query);
			
}

function add_entry($race_id, $division_id, $heatnum, $skate_id, $pos, $min, $sec, $hun, $place, $points, $notes)
{
...
#	echo " index = $i_skater ,value =  $_POST[$i_skater]<br>";
	...
	$notes = $_POST[$i_notes];
	add_entry($RACE_ID, $DIVISION_ID, $HEATNUM, $skate_id, $pos, $min, $sec, $hun, $place, $points, $notes);
	$entry++;
}


echo "<META HTTP-EQUIV=\"Refresh\" 
	CONTENT=\"1; URL=DivisionEnterResults.php?RACE_ID=$RACE_ID&DIVISION_ID=$DIVISION_ID\">";

echo "<a href=DivisionEnterResults.php?RACE_ID=$RACE_ID&DIVISION_ID=$DIVISION_ID>Updating database.... Click this link in case the browser gets stuck here</a>";
?>
</body>
</html>
