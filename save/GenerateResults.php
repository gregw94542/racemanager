...
function display_points_footer()
{
   ...
   echo " For official uses (qualifying times for National level competitions, etc.), each time shall have .20 seconds (2 tenths of a second)<br>";
   ...
   echo "DQ times listed are for training purposes only, and may NOT be used for any Official reasons.";
   echo "</p>";
}
...
function display_race($id) {
		...
		while ( $row = mysql_fetch_array($result,MYSQL_ASSOC)){	
			...
			if ($current_did != $row[did]) {
				...
				###################################################################
				#echo "</tr><tr><td><h1>&nbsp</h1></td></tr>";
				echo "</tr>";
				echo "<tr><td colspan=5><h2>$division_name</h2></td></tr>\n";
				$current_place = 1;
			...
			if ($current_id != $row[id]) {
				$current_id = $row[id];
				echo "\n<tr><td class=\"l_info\">$current_place</td><td class=\"l_info\" width=100px>$row[firstname] $row[lastname]</td>";
